// Javascript Web Rendering and Pagination
...
addUFOBtn.addEventListener("click", function(event) {
    ...
    SearchedData=[];
    for (var i = 0; i < dataSet.length; i++) {
        if (dataSet[i]["datetime"]==dttime || dttime=="")
        {
           SearchedData.push(dataSet[i]);
        }
    }
...
addUFOMultiBtn.addEventListener("click", function(event) {
    ...
    for (var i = 0; i < dataSet.length; i++) {
        if  ((dataSet[i]["datetime"]==dttime || dttime=="") &&
                (dataSet[i]["city"].toUpperCase()==city.toUpperCase() || city=="") &&
                (dataSet[i]["state"].toUpperCase()==state.toUpperCase() || state=="") &&
                (dataSet[i]["country"].toUpperCase()==country.toUpperCase()|| country=="") &&
                (dataSet[i]["shape"].toUpperCase()==shape.toUpperCase() || shape=="") 
                ) 
        {
            SearchedData.push(dataSet[i]);
        }
